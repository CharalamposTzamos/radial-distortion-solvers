...
namespace gcransac
{
	namespace estimator
	{
		namespace solver
		{
			...
			class Radial_10Point : public SolverEngine
			{
			public:
				double loss_scale;
			...
			OLGA_INLINE bool Radial_10Point::estimateModel(
				const cv::Mat& data_,
				...
				const double *weights_) const
			{
				...
				for (size_t i = 0; i < n_sols; i++) {
					...
					model.descriptor << 
                        Fs.col(i)[0], Fs.col(i)[1], Fs.col(i)[2], Ls(0, i),
						Fs.col(i)[3], Fs.col(i)[4], Fs.col(i)[5], Ls(1, i),
						Fs.col(i)[6], Fs.col(i)[7], Fs.col(i)[8], 0;
